...
public class Graph {
    ...
    public Graph prims(int start) {
        HashMap<Integer, Edge> distFromTree = new HashMap<>(); //Initialize distance map
        Graph T = new Graph(); //Initialize MST
        //Initialize Priority Queue w/ minimum edge weight comparator
        PriorityQueue<Integer> queue = new PriorityQueue<>((v1, v2) ->
                distFromTree.get(v1).getWeight() - distFromTree.get(v2).getWeight());
        queue.add(start); //Add start vertex to queue
        while (T.allEdges.size() < this.getAllEdges().size()) { //Continue until T has V-1 edges
            int minVertex = queue.poll(); //Poll vertex from Graph with minimum edge weight to MST
            T.addVertex(minVertex); //Add vertex to MST
            for (Edge e : getEdges(minVertex)) {
                int neighbor = e.getDest();
                if ((!T.containsVertex(neighbor)) && (!distFromTree.containsKey(neighbor))
                        || distFromTree.get(neighbor) == null
                        || e.getWeight() < distFromTree.get(neighbor).getWeight()) {
    ...
    public Graph kruskals() {
        Graph T = new Graph();
        for (Integer v: this.neighbors.keySet()) {
            T.addVertex(v);
        }
        WeightedQuickUnionUF vSet = new WeightedQuickUnionUF(T.neighbors.keySet().size());
        for (Edge e: this.getAllEdges()) {
            vSet.union(e.getSource(), e.getDest());
        }
        List<Edge> edgeList = new ArrayList<Edge>(this.getAllEdges());
        Collections.sort(edgeList);
        for (int i = 0; i < edgeList.size(); i++) {
            Edge e = edgeList.get(i);
            if (vSet.find(e.getSource()) != vSet.find(e.getDest())) {
                T.addEdge(e);
            }
        }
        return T;
    ...
    public static void main(String[] args) {
        Graph g = Graph.randomGraph(10, 9, 20);
        System.out.println(g.getAllEdges());
        System.out.println(g.prims(g.getAllVertices().first()).getAllEdges());
    }
}
